...
def generate_synthetic_sla_labels(df: pd.DataFrame) -> np.ndarray:
    ...
    labels = []
    breach_rates = {"Critical": 0.60, "High": 0.35, "Medium": 0.15, "Low": 0.05}
...
def build_sla_feature_dicts(df: pd.DataFrame, queue_lengths: np.ndarray) -> list:
...
def train_all_models(
    ...
    cat_clf.train(X_train_feat, y_cat_train, use_grid_search=use_grid_search)
    ...
    cat_val_report = classification_report(
        y_cat_val, y_cat_pred_val, target_names=ALL_CATEGORIES, output_dict=True
    )
    cat_test_report = classification_report(
        y_cat_test, y_cat_pred_test, target_names=ALL_CATEGORIES, output_dict=True
    )
    ...
    y_pri_pred_labels = pri_clf.label_encoder.inverse_transform(y_pri_pred_test)
    pri_f1_macro = f1_score(
        y_pri_test, y_pri_pred_labels, average="macro", zero_division=0
    )
    ...
    if not skip_sla:
        ...
        sla_probs = [sla_clf.predict_breach_probability(t) for t in test_sla_dicts]
        try:
    ...
    targets = {
        ...
        "Security Recall >= 0.98": cat_test_report.get("Security", {}).get("recall", 0)
        >= 0.98,
        "Billing F1 >= 0.88": cat_test_report.get("Billing", {}).get("f1-score", 0)
        >= 0.88,
    ...
    if all_passed:
        ...
        logger.warning("Some targets not yet met — expected with synthetic-only data.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train TicketFlow AI models")
    parser.add_argument(
        "--data-dir", default=None, help="Path to directory with Kaggle CSV datasets"
    )
    parser.add_argument(
        "--skip-grid-search",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-sla", action="store_true", help="Skip SLA predictor training"
    )
