...
def prepare_suraj520_dataset():
    ...
    if subject_col and desc_col:
        df["combined_text"] = (
            df[subject_col].fillna("") + " " + df[desc_col].fillna("")
        ).str.strip()
    elif desc_col:
    ...
    logger.info(
        f"✓ After removing duplicates: {len(df)} rows (removed {before_dedup - len(df)})"
    )
    ...
    print(
        f"Removed:           {initial_count - len(df_final)} ({100*(initial_count-len(df_final))/initial_count:.1f}%)"
    )
