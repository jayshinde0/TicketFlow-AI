...
class FeedbackEdit(BaseModel):
    ...
    corrected_category: Optional[str] = Field(
        default=None,
        description="If the category was wrong, provide the correct one"
    )
    corrected_priority: Optional[str] = Field(
        default=None,
        description="If the priority was wrong, provide the correct one"
    )
...
class FeedbackReject(BaseModel):
    ...
    manual_response: str = Field(
        min_length=10, max_length=2000,
        description="The manually crafted response"
    )
    corrected_category: Optional[str] = None
    corrected_priority: Optional[str] = None
    rejection_reason: Optional[str] = Field(
        default=None, max_length=500,
        description="Why the AI suggestion was rejected"
    )
