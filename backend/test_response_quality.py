# backend/test_response_quality.py
...
def test_vpn_response_quality():
    ...
    print(f"📁 Category: {ai_analysis['category']} (confidence: {ai_analysis['model_confidence']:.2f})")
    ...
    response_text = ai_analysis.get('generated_response', 'N/A')
...
def score_response(response: str, category: str) -> float:
    ...
    if category == "Network":
        ...
        vpn_keywords = ['firewall', 'port', 'gateway', 'routing', 'ping', 'dns', 'mtu', 'ike', 'ipsec']
        if any(kw in response_lower for kw in vpn_keywords):
            score += 2.0  # +2 for technical depth
        
        # Log/diagnostic keywords
        log_keywords = ['log', 'event viewer', 'appdata', 'var/log', 'command', 'test', 'check']
        if any(kw in response_lower for kw in log_keywords):
            score += 1.5  # +1.5 for diagnostics
        ...
        score -= generic_count * 0.5  # -0.5 for each generic phrase
    
    # General scoring
    if 'escalat' in response_lower:
        score -= 0.5  # Penalize premature escalation
    
    # Bonus for structured format
    if response.count('\n') > 3:
        score += 0.5  # +0.5 for good formatting
...
if __name__ == "__main__":
    ...
    if score >= 4.0:
        print("\n✅ RESPONSE QUALITY EXCELLENT - Using Qwen-2.5-Coder")
    elif score >= 3.0:
        print("\n✓ RESPONSE QUALITY GOOD - Consider Qwen-2.5-Coder")
    else:
        print("\n❌ RESPONSE QUALITY POOR - Switch to Qwen-2.5-Coder NOW")